...
func parseConfig(logger *zap.Logger) config {
	...
	// Flags
	var (
		bindAddr             = flag.String("bindAddr", "0.0.0.0", `Local interface address to bind to. "localhost" only allows access from the local host. "0.0.0.0" binds to all network interfaces.`)
		port                 = flag.Int("port", 5000, "Port to listen on")
		baseURL              = flag.String("baseURL", "0.0.0.0:5000", "Base URL of this service. It's used in a stream URL that's delivered to Stremio and later used to redirect to RealDebrid, AllDebrid and Premiumize. If you enable OAuth2 handling this will also be used for the redirects and to determine whether the state cookie is a secure one or not.")
